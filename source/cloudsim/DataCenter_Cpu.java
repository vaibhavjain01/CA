...
public class DataCenter_Cpu extends CloudSim {
	...
	protected VMProvisioner_Cpu vmprovisioner;
     ...
     * @post $none
     */
	public DataCenter_Cpu(String name, DatacenterCharacteristics_Cpu resource, VMProvisioner_Cpu vmprovisioner, LinkedList<Storage> storageList) throws Exception {
		super(name);
		resource_ = resource;
   ...
   /**
	...
	protected void processDataAdd(Sim_event ev, boolean ack) {
	       ...
	       if (ev == null) {
	        ...
	        debit+=((DatacenterCharacteristics_Cpu)resource_).getCostPerBw()*file.getSize();
	...
	protected void processVMCreate(Sim_event ev, boolean ack){
    	...
    	VMCharacteristics description = (VMCharacteristics)ev.get_data();
 	    ...
 	    if(result){
 	    	...
 	    	amount+=((DatacenterCharacteristics_Cpu)resource_).getCostPerMem()*description.getMemory();
 	    	amount+=((DatacenterCharacteristics_Cpu)resource_).getCostPerStorage()*description.getSize();
    ...
    protected void processCloudletSubmit(Sim_event ev, boolean ack) {
        ...
        try {
            ...
            cl.setResourceParameter(super.get_id(), resource_.getCostPerSec(), ((DatacenterCharacteristics_Cpu)resource_).costPerBw);
            ...
            Host_Cpu host = vmprovisioner.getHost(vmId,userId);
	...
	protected void processCloudletCancel(int cloudletId, int userId, int vmId) {
        ...
        if (cl != null) {
            ...
            try {
                ...
                cl.setResourceParameter(this.get_id(), resource_.getCostPerSec(),((DatacenterCharacteristics_Cpu)resource_).getCostPerBw());
            } catch(Exception e) {
                // empty ...
            }
        }
	...
	protected void updateCloudletProcessing(){
		...
		if(GridSim.clock()>this.lastProcessTime){
			MachineList_Cpu list = vmprovisioner.getResources();
			double smallerTime = Double.MAX_VALUE;
			//for each host...
			for(int i=0;i<list.size();i++){
				Host_Cpu host = (Host_Cpu) list.get(i);
