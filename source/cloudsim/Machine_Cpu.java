 ...
 * @invariant $none
 */
public class Machine_Cpu
{
    // |PEs| > 1 is SMP (Shared Memory Multiprocessors)
    private PEList_Cpu PEList_;
    private int id_;
     ...
     * @post $none
     */
    public Machine_Cpu(int id, int numPE, int ratingPE) {
        this.id_ = id;
        failed_ = false;
        
        PEList_ = new PEList_Cpu();
        for(int peId=0; peId<numPE; peId++) {
        	PEList_.add(new PE_Cpu(peId, ratingPE));
        }
    }
     ...
     * @post $none
     */
    public Machine_Cpu(int id, PEList_Cpu list)
    {
        this.id_ = id;
        this.PEList_ = list;
        failed_ = false;
    ...
    public int getMIPSRating()
    {
        ...
        Iterator<PE_Cpu> it = PEList_.iterator();
        while ( it.hasNext() )
        {
            obj = it.next();
