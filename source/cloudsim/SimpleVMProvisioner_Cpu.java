 ...
 * @invariant $none
 */
public class SimpleVMProvisioner_Cpu extends VMProvisioner_Cpu {
	
	protected HashMap<String, Host_Cpu> vmTable;
	 ...
	 * @post $none
	 */
	public SimpleVMProvisioner_Cpu() {
		vmTable = new HashMap<String, Host_Cpu>();
	 ...
	 */
	@Override
	public void init(MachineList_Cpu list){
		
		super.init(list);
		freePEs = new int[resources.size()];
		for (int i=0;i<freePEs.length;i++) freePEs[i]=((Host_Cpu)resources.get(i)).getNumPE();
	...
	public boolean allocateHostForVM(VMCharacteristics vm) {
		...
		if(!this.vmTable.containsKey(vm.getVmId()+"-"+vm.getUserId())){//if this vm was not created
			...
			do{//we still trying until we find a host or untill we try all of them
				...
				Host_Cpu host = (Host_Cpu)resources.get(idx);
	...
	public void deallocateHostForVM(int vmID, int userID) {		
		Host_Cpu host = vmTable.remove(vmID+"-"+userID);
	...
	public boolean migrateVM(int vmID, int userID, int newHostID) {
		...
		Host_Cpu source = getHost(vmID,userID);
		VirtualMachine vm = source.vmMigrate(vmID, userID);
		if(vm==null) return false;
		
		Host_Cpu destination = (Host_Cpu) resources.getMachine(newHostID);
	 ...
	 */
	@Override
	public Host_Cpu getHost(int vmID, int userID) {
