 ...
 * @invariant $none
 */
public class CpuSharedAllocationPolicy extends VMMAllocationPolicy {
	 ...
	 * @post $none
	 */
	public CpuSharedAllocationPolicy(PEList_Cpu pelist) {
		super(pelist);
	...
	public double[] getMIPSShare(int vmId, int userId) {
		...
		//divides available MIPS among all VMs
			double[] myShare = new double[pelistcpu.size()];
			for(int i=0;i<myShare.length;i++) myShare[i]=0;

			double capacity=0.0;
			for (int i=0;i<pelistcpu.size();i++){
				capacity+=((PE_Cpu)pelistcpu.get(i)).getMIPSRating();
